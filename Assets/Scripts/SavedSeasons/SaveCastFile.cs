...
public class SaveCastFile : MonoBehaviour, IPointerDownHandler {
    public Text output;
    ...
    public void OnPointerDown(PointerEventData eventData) {
        var bytes = Encoding.UTF8.GetBytes(JsonUtility.ToJson(cast));
    ...
    public void OnClick() {
        string nameNoSpaces = GameManager.Instance.seasonTemp.nameSeason.Replace(" ", "") + "Cast";
        ...
        if (!string.IsNullOrEmpty(path)) {
            File.WriteAllText(path, JsonUtility.ToJson(cast));
        }
    }
#endif
